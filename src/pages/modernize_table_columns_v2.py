...
if start_idx != -1 and end_idx != -1:
    ...
    # Identify the columns block to replace
    # It starts with TABLE COLUMNS Typography and ends with the columns map
    
    # We'll use a landmark approach
    landmark_start = "<Typography variant=\"caption\" sx={{ fontWeight:700, color:'#546E7A', letterSpacing:.5 }}>TABLE COLUMNS</Typography>"
    landmark_end = "{(field.columns||[]).map((col,ci)=>("
    
    # Find the parent box of the landmark
    # It's <Box sx={{ mb: 2 }}> containing TABLE COLUMNS
    
    # Let's just reconstruct the whole Case 3 again with the new UI
    # It's the most reliable way given my tool issues.
...
print("Table Columns UI Modernized (Total Block Replacement)")
